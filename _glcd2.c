...
//			x++;
			...
			/* TEST
			while(1){//test
				GLCD_Rectangle_black(0,0,63,127);
				_delay_ms(1000);
				lcd_clear_part(0,0,63,256);
				_delay_ms(1000);

				GLCD_Rectangle_black(0,0,63,127);
				_delay_ms(1000);
				lcd_clear_part(0,0,30,256);
				_delay_ms(1000);
				
				GLCD_Rectangle_black(0,0,63,127);
				_delay_ms(1000);
				lcd_clear_part(32,0,63,256);
				_delay_ms(1000);
			}*/
	}
	/* GLCD Initialize */
	...
	unsigned char ScreenBuffer_map[8][128];	//맵저장용
	// draw a dot on GLCD
	// 점을 그립니다.
	...
	//만든 함수
	/* bit map draw by dot */
	void glcd_draw_bitmap(const unsigned char *bmp,unsigned char x, unsigned char y, unsigned char dx, unsigned char dy)
	{
		unsigned char i,j;
	...
	void GLCD_Restore_buffer(unsigned char x1,unsigned char y1,unsigned char x2,unsigned char y2) {
		int x, y;
		for(x = x1; x <= x2; x++) {
			for(y = y1; y <= y2; y++) {
				GLCD_Restore_dot(x,y);
			}
		}
	}
	void GLCD_Restore_dot(unsigned char xx,unsigned char y){
		unsigned char x;
		...
		x = xx / 8;                 // calculate x address
		GLCD_Axis_xy(x, y);          // draw dot on GLCD screen
		if(y <= 63) { datal(ScreenBuffer_map[x][y]);  }
		else        { datar(ScreenBuffer_map[x][y]);  }
	}
